...
// on. It is a rename at the seam, not a wrapper: every transition returned here
// is the core function itself, unwrapped, because the no-op paths return their
// input BY REFERENCE and `main.js` and the renderer's pulse guard compare
// identity to detect that nothing happened. A wrapper would hand back a fresh
// object each time and break both, silently — the game would keep working and
// merely re-run the shake animation on every frame.
//
...
const engine = createEngine(mathSpace, { itemKey: 'fact' });
